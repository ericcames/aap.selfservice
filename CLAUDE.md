...
- `sync_portal_orgs.yml` >5m — the new pod is failing readiness; `oc logs -n aap-portal deploy/rhaap-portal -c backstage-backend --previous` is the first place to look. The playbook now blocks on `oc rollout status` so it shouldn't return until the new ReplicaSet is fully serving — if it does return early, that's a regression.
