...
### Fixed
- CLAUDE.md "Expected Timings" total wall clock corrected from ~12 min to ~11 min so it sums from the line items. The ~12 min figure was from the original broken-sync run where the rollout wait happened externally; with #38 the wait is inside `sync_portal_orgs.yml` and the line-item sum is ~11 min (resolves #40)
- `sync_portal_orgs.yml`: replaced "Wait for portal to be ready" (which used `availableReplicas >= 1`) with `oc rollout status` so the playbook blocks until the new ReplicaSet has fully cut over. The old check was satisfied throughout a rolling update because the old pod stayed Running, so the playbook returned while the route was still serving traffic from the pre-patch pod — users hitting the portal in that window got `does not belong to any configured organizations` errors. Verified against `rhdp-ames-test2` 2026-05-06: pre-fix duration 5s (returned mid-rollout), post-fix duration 233s (returned after cutover). CLAUDE.md "Expected Timings" updated to reflect the merged duration (resolves #38)
- Documentation references aligned with current playbook implementation: `references/context.md` OAuth applications endpoint corrected to the gateway path; CLAUDE.md Architecture section corrected (OAuth API is gateway not controller, OCP secret names match what `bootstrap_portal.yml` actually creates, `bootstrap_aap.yml` execution-order line reflects current scope after vault removal); ROADMAP Phase 2 checklist and Collections line updated to drop the vault credential and OAuth-app entries (OAuth lives in `bootstrap_portal.yml`); ROADMAP Phase 3 stale "Tasks" planning subsection removed and the catalog-providers override note preserved by promoting it into "Key findings" (resolves #30)
